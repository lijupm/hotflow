...
class PatientController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $patients = $em->getRepository('HotfloORBundle:Patient')->findAll();
    ...
    {
        $patient = new Patient();
        $form = $this->createForm('Hotflo\ORBundle\Form\PatientType', $patient);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('patients_show', array('id' => $patient->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($patient);
        $editForm = $this->createForm('Hotflo\ORBundle\Form\PatientType', $patient);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('patients_edit', array('id' => $patient->getId()));
        }
    ...
    private function createDeleteForm(Patient $patient)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('patients_delete', array('id' => $patient->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
